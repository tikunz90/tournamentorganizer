...
addMinutesButtons.forEach(function(button) {
  button.addEventListener("click", function() {
    ...
    $table.find('tr').each(function(index) {
      ...
      var $row = $(this);
      var gameId = parseInt($row.find("#game_id_counter").attr("data-content"));
      
      if (gameId == selectedGameId) {
        console.log("Found selected game")
        minutesValue = $("#minutes-input").val();
        console.log("Start Adding " + minutesValue);
        actDay = moment($row.find('#id_starttime')[0].value, "MM/DD/YYYY HH:mm").date();
        gameFound = true;
      }
      if(gameFound)
      {
        
        var actDate = moment($row.find('#id_starttime')[0].value, "MM/DD/YYYY HH:mm");
        if(actDay == actDate.date())
        {
          console.log("Adding " + gameId);
          var newDate = actDate.clone().add(minutesValue, 'minutes');
          var gameJson = {'game': gameId,
          'datetime': newDate.format("YYYY-MM-DD HH:mm:ss")
          };
          //postUpdateGameStartTime(gameJson, gameJson);
          var data = postUpdateGameDateTime(gameId, newDate.format("YYYY-MM-DD HH:mm:ss"));
...
$(".game-list-datetime" ).each(function(i, datetime_input){
  ...
  $(datetime_input).val(date_string);
      ...
      $('.game-row').click(function() {
        ...
        selectedRow = $(this);
        var date_string_dragged = moment(selectedRow.find('#id_starttime')[0].value, "MM/DD/YYYY HH:mm").format("YYYY-MM-DD HH:mm:ss");
        selectedGameId= selectedRow.find('#game_id_counter')[0].getAttribute("data-content");
  ...
  function attachEventListeners() {
    ...
    var menuItems = document.querySelectorAll(".context-menu li");
      ...
      var buttons = document.querySelectorAll(".modal-gameplan-row-selection-content button");
      ...
      function postUpdateGameDateTime(game_id, new_datetime) {
        return $.ajax({
            ...
            success:function(response){
              ...
              $(game_td).children('.game-list-datetime-input').find('#id_starttime')[0].value = date_input;
              },
            complete:function(){console.debug('complete');},
            error:function (xhr, textStatus, thrownError){console.debug('error');}
          });
