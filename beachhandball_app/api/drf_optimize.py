...
class OptimizeRelatedModelViewSetMetaclass(type):
    ...
    Which is done by using the fields declared on the Serializer 
    to determine ForiegnKey, OneToOneField and ManyToMany fields,
    that should be added to `prefetch_related` and `select_related`.
    ...
    and has a `Meta.fields` property, then all fields are used to optimize the view queryset.
    """
